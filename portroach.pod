...
This will instruct Portroach to search for new distfile for each port in the
...
Portroach doesn't handle ports with multiple distfiles. It uses ROACH_URL which
is the first(main) distfile chosen by bsd.port.mk from the port's Makefile.
